...
The actor then makes an explicit new route decision from factual origin `hex:hub` to `hex:destination` on day 21. Under the same scenario policy, the remaining estimates received day 20 are fresh (age one day), and D selects `hub → west → destination` (6 estimated days). This expected result depends on the KnownUnavailable eligibility rule accepted in the revised P8-D design cited above: the day-21 observation makes the attempted `hub → east` option ineligible for route candidate selection. That rule is accepted design semantics, but still awaits implementation and promotion in the P8-D capability. P8-E does not impose that filtering itself. If the promoted D capability does not implement the accepted rule (or equivalent behavior), this proving replan is blocked for D-owner resolution rather than silently assuming the alternate wins. P8-E accepts the new plan, starts each validated segment, advances deterministically using the exact promoted C progress API, and commits `At(hex:destination)` plus plan completion through the same travel transaction boundary. No route is switched automatically as a side effect of failure.
...
- replan on day 21 is a new explicit decision from `hex:hub`, selects only the alternate known candidate under fresh supplied estimates when the accepted P8-D KnownUnavailable eligibility rule is implemented and promoted, and never auto-switches during rejection;
