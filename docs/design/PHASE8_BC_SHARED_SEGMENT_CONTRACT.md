...
Persistent Person position and trip origin/destination use only stable typed references: `HexId`, `LocationId`, or `CrossingId`. The existing D0 `SpatialReference` handles Hex and Location; the spatial foundation must add Crossing as a typed reference using stable `CrossingId` and validate resolution through the same world-bound spatial authority. P8-C must reject `SubLocation` references in Person position and trip state because the current D0 representation identifies its topology owner/place with runtime IDs and cannot satisfy persistence or reconstruction. D0 SubLocation references may continue serving existing non-persistent consumers.

A `Location` resolves to its one P8-A `AnchorHexId`; a `Crossing` resolves to its registered physical anchor/boundary facts. Resolution reports its typed reference and regional Hex context. It does not imply a local entrance or a traversable path between that reference and the resolved Hex. Existing SubLocation resolution remains available only through its current explicit LocalTopology binding for non-persistent consumers. Any later persistent SubLocation support requires stable semantic SubLocation and owner IDs plus a resolver contract that can reconstruct those IDs independently of runtime allocation.

The shared contract does not add a general local-routing graph. Moving between a City/Site anchor and a Hex or between two POIs in one Hex requires an explicit local topology/entry transition if the travel slice needs it. Same-Hex membership alone never supplies that transition. P8-C maps legacy City/Site identity to the already-authoritative neutral anchor; it does not manufacture access links or migrate all of LocalTopology.
...
```text
At(StablePositionReference)
InTransit(TraversalProgress)
```

`StablePositionReference` is a closed P8-C value over `HexId`, `LocationId`, or `CrossingId`; it is not the general D0 `SpatialReference` union. P8-C position and trip APIs reject SubLocation references and any reference whose identity depends on `RuntimeId`. `TraversalProgress` records the stable traversal-option reference, the canonical boundary key, ordered direction (`FromHexId` → `ToHexId`), and a deterministic progress value sufficient to resume at the simulation’s supported temporal granularity. It also identifies the last fully reached stable position reference/Hex. It contains no Unity transform, RuntimeId, or continuous rendering coordinate. It must resolve to a registered factual option/boundary at the point of mutation and preserve the single current position; neither endpoint is treated as already reached until the authoritative arrival transition commits.
...
Crossing a boundary is an atomic position transition from `At(origin reference)` through `InTransit(...)` to `At(destination reference)`. The physical position store is the only source for “where is this Person now?” Position changes must validate the Person identity and stable spatial references before mutation and must leave state/revision unchanged on rejection. Position is not inferred from residence, a City’s legacy `Location` object, loaded NPC state, or the accepted destination. Any later support for persistent SubLocation positions is a separate extension requiring stable semantic SubLocation and topology-owner identities and a resolver contract; runtime-only D0 references cannot be persisted or reconstructed as Person truth.
...
| P8-C Legacy Anchors and Civil Presence | City/Site ↔ `LocationId` bridge; `PersonId`-keyed position authority and typed `At(StablePositionReference)`/`InTransit` values; position cloning, guards, invariants/diagnostics and focused presence tests. Own a new source such as `PersonSpatialPositionStore.cs`; alter `CityRuntime` / `ExplorableSiteRuntime` only for explicit anchor binding. Test that position/trip APIs reject current runtime-only SubLocation references and that accepted Hex/Location/Crossing/transit identities reconstruct from stable IDs. | Passage truth/condition/query implementation, route choice, `PersonRuntime` duplicate spatial collections, `NpcRuntime`-only position, persistent SubLocation support, LocalTopology migration, or daily loop changes. It must not make same-Hex an access link. |
