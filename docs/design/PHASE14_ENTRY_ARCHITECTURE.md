...
**Observed code base:** `codex/phase14/MaterialFlowEntryArchitecture`, based on `1f4651e99db2c357dd3be3c6b9284d104379f706` (`codex/phase7/canonical`). The current dependency review uses `docs/PHASE8_STATE.md` from `codex/phase8/canonical` at `ed7a40a86a6a16e9f4fda75703470c38135fda0e`: P8-A through P8-C are canonical; P8-D is ready for implementation and P8-E waits on D. The Phase 14 Brief is `ENTRY_ARCHITECTURE_READY`; no Phase 14 checkpoint IDs are approved. This document intentionally chooses neither the first consumer nor the closure boundary.
...
| Spatial authority and travel | At the observed P7 code base, typed `HexId`/`LocationId`, `LocationRecord → AnchorHexId`, and world-owned `SpatialAuthorityStore` were present; the P7 State left City/Site mapping and route/travel rewrite deferred. Current P8 state at `ed7a40a` promotes P8-A geography/terrain identity and revisions, P8-B passage facts, and P8-C runtime-composed City/Site anchors plus Person `At`/`InTransit` truth and diagnostics. The P7-base `CityRuntime` and `TravelSystem` observations use legacy `SpatialLocationRuntime`, direct `SpatialRouteRuntime`, and fixed `TravelDays`. | Existing P8-A/B/C authorities are available dependencies, including local City/Site anchors. P8-D is ready but not implemented/promoted; P8-E still waits on D, so Knowledge-bounded civil travel behavior is not yet available. A route-dependent P14 implementation must wait for the specific promoted P8 travel capability it needs. The P14 Brief does not require every P8-E behavior for a local source. Legacy fixed-day routes are not a second authority for new route-dependent material flow. |
...
2. **A localized resource source feeding a settlement or other stock holder.** Examples could be an authored mine, forest, farm, or other site, but none is represented by a general productive-source runtime in the inspected code. This creates the clearest source identity/location/ownership question. A source that uses a current Location, City, or Site anchor can build on the promoted P8-A/C authority; any additional footprint or route behavior must name its required promoted P8 capability.
...
Entry must settle at least: (a) who owns the source itself; (b) who owns output on creation; (c) whether output owner is distinct from worker/producer; (d) who has custody and where stock is held while carried/stored; (e) who has factual control or authorization to operate/extract from a source, whether that differs from owner, producer, custodian, carrier, or store, and which current facts execution must revalidate; (f) what transfer changes title versus only custody/location; and (g) whether populations or institutions can produce/own without materializing a Person or NpcRuntime. These are domain choices; this packet does not prescribe a universal rights model. A technical design can then map them onto stores and transition boundaries.

## 6. Spatial and travel dependencies

- A source described as localized must reference the approved physical location/anchor authority rather than inventing a City-only or renderer coordinate. P7 provides typed spatial records; current P8-C has promoted runtime-composed City/Site anchor bindings and P8-A supplies Hex/Location and terrain identity. A source using those anchors can depend on the current P8 canonical authority. Do not treat the older `SpatialLocationRuntime` bridge as equivalent.
...
- If a flow physically travels between locations, the flow must join the same spatial/travel authority used by that trip. P8-A/B/C provide geography, passage truth, anchors, and Person position; P8-D/E travel capabilities are not yet promoted at the cited state. The current legacy `SpatialRouteRuntime.TravelDays` path is not an alternate P14 route authority. Route-dependent implementation waits for the needed P8 travel capability to be promoted.
...
5. **Owner, custodian, and control authority:** which holder kinds can own a source and its output in v1; whether Person/aggregate/institution holders are included; whether the owner may differ from producer, market, store, carrier, or actor with control; who may operate/extract from a source; which operations change title versus custody/location; and what current facts an execution must revalidate for source access. This is a bounded product/domain choice, not a request for a universal rights system.
...
7. **Spatial scope:** whether the selected source needs a Location anchor only, a Hex-bound source or footprint, or route travel; for travel, which promoted P8 capability the design consumes. Current P8-A/B/C geography, passage, and anchor/presence authorities are canonical; P8-D/E travel capabilities are not yet promoted.
